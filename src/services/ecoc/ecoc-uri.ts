...
export default class EcocURI implements URI {
  ...
  _parseAmount(amount: string): string {
  ...
  static fromString(str: string): EcocURI {
  ...
  static fromObject(object: URI): EcocURI {
    return new EcocURI(object)
  }

  static isValid(uri: string | URI): boolean {
    try {
      new EcocURI(uri)
    } catch (err) {
      return false
